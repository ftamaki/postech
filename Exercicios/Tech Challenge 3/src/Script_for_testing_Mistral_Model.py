...
def testar_modelo_lora(adapter_path):
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        device_map={"": 0} if device == "cuda" else "auto", # Sempre usar device_map="auto" para carregar o modelo corretamente
...
if __name__ == "__main__":
    ...
    model_path = script_dir.parent / "trained_data" / "fine_tuned_model_lora" / "med-assistant-mistral-lora"
    if model_path.exists():
        testar_modelo_lora(str(model_path))
    else:
        print(f"ERRO: Caminho não encontrado: {model_path}")
        print(f"Verifique se a pasta 'med-assistant-mistral-lora' existe em: {script_dir.parent / 'trained_data'}")
